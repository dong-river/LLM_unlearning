...
class GenerationMixin:
    ...
    def generate(
        ...
        **model_kwargs,
        ...
        print("Using Lisa version of generate function")
        ...
        if self.config.is_encoder_decoder and "encoder_outputs" not in model_kwargs:
            print("setting encoder-decoder model config at line 1218 in generation_utils.py. This is not expected.")
        ...
        is_sample_gen_river_mode = (
            (num_beams == 1) and (num_beam_groups == 1) and do_sample is True and not is_constraint_gen_mode
        )
        is_beam_gen_river_mode = (
            (num_beams > 1) and (num_beam_groups == 1) and do_sample is False and not is_constraint_gen_mode
        )
        is_greedy_gen_river_mode = (
            (num_beams == 1) and (num_beam_groups == 1) and do_sample is False and not is_constraint_gen_mode
        )
        ...
        if is_greedy_gen_mode:
            ...
            print('is_sample_gen_mode in line 1405 generation_utils.py')
        ...
        elif is_beam_gen_river_mode:
            pass
        
        elif is_greedy_gen_river_mode:
            pass
        ...
        **model_kwargs,
        ...
        while True:
            ...
            if model_kwargs['teacher_student']:
                ...
                print(str(logits_warper_student))
            ...
            if model_kwargs['teacher_student']:
                ...
                if model_kwargs_student['strat'] == 'relu':
                    # print("Using Contrastive Decoding, relu startegy")
                    next_token_scores = next_token_scores - model_kwargs['st_coef'] * relu(next_token_logits_student)
                    next_token_scores = logits_processor(input_ids, next_token_scores)
                    next_token_scores = logits_warper(input_ids, next_token_scores)
                elif model_kwargs_student['strat'] == 'relu2':
                    # print("Using Contrastive Decoding, relu startegy")
                    next_token_scores = next_token_scores - model_kwargs['st_coef'] * relu(next_token_logits_student - next_token_scores)
                    next_token_scores = logits_processor(input_ids, next_token_scores)
                    next_token_scores = logits_warper(input_ids, next_token_scores)
                elif model_kwargs_student['strat'] == 'relu3':
                    print("Using Contrastive Decoding, relu3 startegy")
                    ## First align the mean of two distributions
                    mean_shift = torch.mean(next_token_logits_student, dim=-1) - torch.mean(next_token_logits, dim=-1)
                    next_token_logits_student = next_token_logits_student - mean_shift.view(-1,1)
                    std_shift = torch.std(next_token_logits_student, dim=-1) / torch.std(next_token_logits, dim=-1)
                    next_token_logits_student = next_token_logits_student / std_shift.view(-1,1)
                    diff = next_token_logits_student - next_token_logits
                    std = torch.std(next_token_logits, dim=-1)
                    
                    logits_warper_tmp = LogitsProcessorList()
                    logits_warper_tmp.append(TopKLogitsWarper(top_k=model_kwargs_student['relu3_topk'], min_tokens_to_keep=1))
                    top_p_mask = (logits_warper_tmp(input_ids, next_token_logits) > -10000).float()
                    mask = top_p_mask * (diff > (model_kwargs_student['relu_threshold'] * std.view(-1,1))).float()
                    next_token_scores = next_token_logits - 10000 * mask
                    # next_token_scores = next_token_scores - model_kwargs['st_coef'] * mask * diff
                    next_token_scores = logits_processor(input_ids, next_token_scores)
                    next_token_scores = logits_warper(input_ids, next_token_scores)
                    
                elif model_kwargs_student['strat'] == 'relu_offset':
                    # print("Using Contrastive Decoding, relu offset startegy")
                    relu_student = relu(next_token_logits_student)
                    offset = torch.sum(relu_student, dim=-1) / (torch.sum(relu_student > 0, dim=-1) + 1).to(relu_student.device)
                    offset_matrix = (relu_student == 0) * offset.view(-1,1)
                    next_token_scores = next_token_scores - model_kwargs['st_coef'] * (relu_student - offset_matrix)                ## In the first set of exp, I did next_token_scores - model_kwargs['st_coef'] * relu_student - offset_matrix
                    next_token_scores = logits_processor(input_ids, next_token_scores)
                    next_token_scores = logits_warper(input_ids, next_token_scores)
                else:
                    raise NotImplementedError
            elif model_kwargs.get("DP", False):
                print("Using DP")
                uniform_dist = torch.ones(next_token_scores.shape[0], next_token_scores.shape[1]).to(next_token_scores.device)
                mean = torch.mean(next_token_scores, dim=-1).unsqueeze(-1)
                uniform_dist = uniform_dist * mean
                next_token_scores = model_kwargs['DP_coef'] * next_token_scores + (1-model_kwargs['DP_coef']) * uniform_dist
                next_token_scores = logits_processor(input_ids, next_token_scores)
                next_token_scores = logits_warper(input_ids, next_token_scores)
            else:
                next_token_scores = logits_processor(input_ids, next_token_scores)
    ...
    def sample(
        ...
        **model_kwargs,
        ...
        CD_num_token = model_kwargs.pop('cd_num_token', float("inf"))
        count = 0
        # print("CD_num_token", CD_num_token)
        while True:
            count += 1
            if synced_gpus:
                # Under synced_gpus the `forward` call must continue until all gpus complete their sequence.
            ...
            # def print_quantile(logits): print('min: ', torch.quantile(logits, 0, dim=-1), '\n25%: ', torch.quantile(logits, 0.25, dim=-1), '\nmedian: ', torch.quantile(logits, 0.5, dim=-1), '\n75%:', torch.quantile(logits, 0.75, dim=-1), '\n95%: ', torch.quantile(logits, 0.95, dim=-1), '\n99%: ', torch.quantile(logits, 0.99, dim=-1), '\nmax: ', torch.quantile(logits, 1, dim=-1))
            # next_token_scores = logits_processor(input_ids, next_token_scores)
            # next_token_scores = logits_warper(input_ids, next_token_scores)
            
            # tmp = next_token_scores.clone().detach().cpu().numpy()
            # np.savetxt('my_file.txt', tmp)
 
            # sample
            # print("Using River's version of transformer")
            if CD_num_token < 100:
                print("CD_num_token is disabled !!!")
            if model_kwargs['teacher_student']:
                relu = torch.nn.ReLU()
                if model_kwargs_student['strat'] == 'relu':
                    # print("Using Contrastive Decoding, relu startegy")
